...
export default {
  ...
  async tokenCheck(req, res, next) {
    try {
      ...
      const {data, status} = await request.axios_get(
        `${config.api}/api/applications/${config.clientId}/tokens/check`,
        req.session.user.access_token
      );

      status === 200 ? next() : res.status(400).json({name: 'InvalidCode', message: '无效的 token'});
    } catch (e) {
      return next(e);
    }
  },
  ...
  async login(req, res, next) {
    ...
    try {
      const {data, status} = await request.axios_post(`${config.account_api}/oauth2/access_token`, {
        client_id: config.clientId,
        client_secret: config.clientSecret,
        code: code
      });
      // 以access_token 查询teambition当前用户信息
      let user = await request.axios_get(`${config.api}/api/users/me`, data.access_token);
  ...
  authUser(req, res, next) {
    if (req.session.user) {
      ...
      if (!auth_token) {
        return res.status(400).json({name: 'InvalidCode', message: '无效的 token'});
      }
